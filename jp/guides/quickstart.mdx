...
<Warning>
**curl で試す場合:** 例は `https://api.oneinbox.ai` です。Cloudflare エラーや接続失敗のときは、各コマンドのベース URL を **`http://api.oneinbox.ai:8000`** に置き換えてください — API キーとヘッダーは同じです。curl・Postman・ターミナルのみ（ブラウザ不可）。`https://api.oneinbox.ai/v1/health` が JSON を返すようになったら、ドキュメントの HTTPS URL に戻してください。
</Warning>
