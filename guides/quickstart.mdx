...
<Warning>
**Testing curl today:** Examples use `https://api.oneinbox.ai`. If you get a Cloudflare error or connection failure, replace that base URL with **`http://api.oneinbox.ai:8000`** in each command — same API key, same headers. Use curl, Postman, or Terminal only (not the browser). When `https://api.oneinbox.ai/v1/health` returns JSON, switch back to the HTTPS URL in the docs.
</Warning>
