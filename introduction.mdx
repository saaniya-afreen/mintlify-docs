...
Prefer a visual tool? Import our Postman collection and run the same requests in order:

1. Download `oneinbox.postman_collection.json` from the repo
2. Import into [Postman](https://www.postman.com/)
3. Run folders in order: **Auth → API Keys → Integrations → LLM Models → Agents → Calls**

Variables like `{{api_key}}` and `{{agent_id}}` auto-fill when you use Postman's test scripts.
