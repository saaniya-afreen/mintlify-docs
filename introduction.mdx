...
<CardGroup cols={2}>
  <Card title="Download Postman collection" icon="download" href="/oneinbox.postman_collection.json">
    All endpoints pre-configured — import into Postman and run in order. Variables auto-save as you go.
  </Card>
  <Card title="API reference" icon="code" href="/api-reference/auth/sign-up">
    Try every endpoint in the browser — same requests as the collection.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="API reference" icon="code" href="/api-reference/auth/sign-up">
    Every endpoint with Try It — same as our Postman collection
  </Card>
</CardGroup>
...
## Using curl vs Postman

This guide shows **curl** commands (paste into Terminal on Mac, or Command Prompt / WSL on Windows).

Prefer a visual tool? Use our Postman collection:

<Card title="Download Postman collection" icon="download" href="/oneinbox.postman_collection.json">
  **OneInbox API v1** — Auth, API Keys, Integrations, LLM Models, Agents, Calls, and more.
</Card>

**Import steps:**

1. [Download the collection](/oneinbox.postman_collection.json) (or click the card above)
2. Open [Postman](https://www.postman.com/) → **Import** → upload the file
3. Run folders in order: **Auth → API Keys → Integrations → LLM Models → Agents → Calls**

Variables like `{{api_key}}`, `{{llm_id}}`, and `{{agent_id}}` auto-fill from Postman's test scripts after each request.
